...
from ui.form_filler.form_filler_window import FormFillerWindow
...
def main():
    ...
    # Force light palette so Windows dark mode does not break the UI.
    palette = QPalette()
    ...
    app.setStyleSheet("""
        * { color: #212529; }
        ...
        QLabel { color: #212529; background-color: transparent; }
        QListWidget {
            border: 1px solid #ced4da;
            border-radius: 4px;
        ...
        QLineEdit[readOnly="true"] { background-color: #e9ecef; color: #495057; }
        QTextEdit {
            border: 1px solid #ced4da;
            border-radius: 4px;
            background-color: #ffffff;
        ...
        QCheckBox { color: #212529; background-color: transparent; }
        QCheckBox::indicator {
        ...
        QRadioButton { color: #212529; background-color: transparent; }
        ...
        QTabBar::tab:selected {
            ...
            border-bottom: 3px solid #2c7be5; font-weight: bold;
        }
        QTabBar::tab:hover:!selected { background-color: #dee2e6; }
        QScrollArea { border: none; background-color: transparent; }
        QScrollArea > QWidget > QWidget { background-color: transparent; }
        QScrollBar:vertical {
            background-color: #f0f0f0; width: 10px; border-radius: 5px;
        }
        QScrollBar::handle:vertical {
            background-color: #adb5bd; border-radius: 5px; min-height: 20px;
        }
    ...
    # Migrate existing config files to add any new fields
    ...
    def _open_filler():
        nonlocal _filler_window
        if _filler_window is None:
            _filler_window = FormFillerWindow()
            _filler_window.batch_completed.connect(dashboard.notify_batch_complete)
        _filler_window.show()
        _filler_window.raise_()
        _filler_window.activateWindow()
